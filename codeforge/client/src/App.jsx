...
function AppInner() {
  const { chatId, files, messages, loadChatList, notify } = useSessionsAndChat();
  ...
  return (
    <ChatProvider chatId={chatId} settings={settings} notify={notify} files={files}>
      <FilesProvider chatId={chatId} notify={notify}>
        <SessionsProvider chatId={chatId} files={files} messages={messages} notify={notify} loadChatList={loadChatList}>
          <UIProvider>
            <Shell />
          </UIProvider>
        </SessionsProvider>
...
function useSessionsAndChat() {
  const sessions = useSessions();
  const chat = useChat();
  return {
    chatId: sessions.openSessions[0]?.id,
    files: [],
    messages: chat.messages,
    loadChatList: sessions.loadChatList,
    notify: sessions.notify,
    ...sessions,
    ...chat,
  };
}
