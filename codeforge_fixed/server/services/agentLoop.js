...
export async function runAgentLoop({
  ...
  async function runConversationsPath() {
    ...
    try {
      ...
      onEvent({ type: "error", message: `Не удалось создать/получить Agent в Mistral: ${err.message}` });
    ...
    try {
      if (canAppend) {
        ...
        ({ outputs } = await appendConversation({
        ...
        const started = await startConversation({ agentId, inputs: seedInputs });
        outputs = started.outputs;
        if (chatId) conversationCache.set(chatId, { conversationId: started.conversationId, agentKey });
      }
    } catch (err) {
      onEvent({ type: "error", message: `Mistral Conversations API error: ${err.message}` });
      return await finalizeTurn();
    }
    ...
    while (convLoops < MAX_LOOPS) {
      ...
      if (text) onEvent({ type: "reasoning", text });
      ...
      try {
        ...
        ({ outputs } = await appendConversation({ conversationId: convId, inputs: results }));
      } catch (err) {
        onEvent({ type: "error", message: `Mistral Conversations API error: ${err.message}` });
        return await finalizeTurn();
      }
