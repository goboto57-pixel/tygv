...
export async function runAgentLoop({
  ...
  const getAdaptiveLoops = (prompt) => {
    if (/(фикс|поправь|исправь|мелкий|one.?line|small fix)/i.test(prompt) && prompt.length < 120) return 5;
    if (/(простой сайт|одностраничник|лендинг|landing)/i.test(prompt)) return 6;
    if (/(сайт|приложение|app|website|страниц)/i.test(prompt)) return 12;
    if (/(большой|сложный|enterprise|full.?stack|многостраничный)/i.test(prompt)) return 20;
    return 12;
  };
  ...
  async function runConversationsPath() {
    ...
    else if (/(простой сайт|одностраничник|лендинг|landing)/i.test(rawLastPrompt)) adaptiveMax = 4000;
