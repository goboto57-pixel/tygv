...
export async function executeTool(toolName, args, fsMap) {
  switch (toolName) {
    ...
    case "write_files": {
      // Batch write — creates/overwrites several files in ONE tool call
      // instead of one write_file per file. This is the real lever against
      // network round-trip latency on the Conversations API path: a typical
      // "make me a landing page" turn writes index.html + style.css +
      // script.js, which used to cost 3 separate model<->tool round trips
      // and now costs 1. Same validation as write_file, applied per file;
      // partial success is allowed (one bad path doesn't block the rest).
      const files = Array.isArray(args.files) ? args.files : [];
      if (files.length === 0) return { error: "files array is required and must be non-empty" };
      const filesChanged = [];
      const errors = {};
      for (const f of files) {
        const path = f && typeof f.path === "string" ? f.path.trim() : "";
        const content = f && f.content;
        if (!path) { errors["(empty path)"] = "path is required and must be non-empty string"; continue; }
        if (typeof content !== "string") { errors[path] = "content must be a string"; continue; }
        if (path.includes("..") || path.startsWith("/") || path.includes("\\") || path.includes("\0")) { errors[path] = "invalid path"; continue; }
        if (path.length > 300) { errors[path] = "path too long"; continue; }
        if (content.length > 1024 * 1024) { errors[path] = "content too large (max 1MB)"; continue; }
        fsMap.set(path, content);
        filesChanged.push(path);
      }
      if (filesChanged.length > 0) { embeddingIndexCache.delete(fsMap); invalidateReadCache(fsMap); }
      const summary = `Wrote ${filesChanged.length} file(s)${Object.keys(errors).length ? `, ${Object.keys(errors).length} error(s)` : ""}.`;
      return filesChanged.length > 0
        ? { result: summary, filesChanged, ...(Object.keys(errors).length ? { errors } : {}) }
        : { error: `No files written. ${JSON.stringify(errors)}` };
    }
    ...
    case "lint_files": {
      // Batch lint — the missing counterpart to write_files. Verifying a
      // freshly-generated 3-file site used to cost 3 separate lint_file
      // round trips (on top of write + read-back + check_preview); this
      // does all of them in one call. Real time saved, not just perceived.
      const paths = Array.isArray(args.paths) ? args.paths : [];
      if (paths.length === 0) return { error: "paths array is required and must be non-empty" };
      const results = {};
      const missing = [];
      for (const p of paths) {
        const content = fsMap.get(p);
        if (content === undefined) { missing.push(p); continue; }
        const issues = lintText(p, content);
        if (issues.length) results[p] = issues;
      }
      const clean = paths.filter((p) => fsMap.has(p) && !results[p]);
      return { result: { issues: results, clean, missing } };
    }
