...
import { startOrResumeJob, subscribe, generateRunId, abortJob, getActiveRunForChat } from "../services/jobManager.js";
...
router.post("/stream", async (req, res) => {
  ...
  //
  // Guard against two jobs racing on the same chat: a client can end up
  // believing a previous turn is over (SSE dropped by a proxy idle-timeout,
  // page reload racing a switch of model, etc) while the old job is still
  // running detached server-side. Starting a second job on the same chatId
  // in that case means two agents mutate the same files and both try to
  // persist the chat — whichever finishes second silently wins, and the
  // client can end up rendering an inconsistent mix of both runs' events.
  // Since this is a genuinely new turn (not a /resume), the old job is
  // stale by definition — abort it before starting the new one.
  if (chatId) {
    const staleRunId = getActiveRunForChat(chatId);
    if (staleRunId && staleRunId !== runId) {
      console.warn(`[chat] chatId=${chatId} already has running job ${staleRunId}; aborting it before starting new job ${runId}`);
      abortJob(staleRunId);
    }
  }
