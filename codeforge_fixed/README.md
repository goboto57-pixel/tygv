...
- Переменные окружения: `MISTRAL_API_KEY`, `MISTRAL_MODEL`, `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`, `NETLIFY_TOKEN` (опционально — включает кнопку «Опубликовать»)
...
## Публикация готового сайта (Netlify)

Кнопка «Опубликовать» (иконка ракеты) в TopBar собирает текущее дерево файлов проекта в ZIP и деплоит его как статический сайт на Netlify — под аккаунтом, чей `NETLIFY_TOKEN` задан на сервере. Пользователь не вводит собственных ключей.

- Требует `index.html` в корне проекта — если его нет, публикация вернёт понятную ошибку вместо пустого сайта.
- Один чат = один сайт Netlify: `siteId` сохраняется в записи чата, повторная публикация обновляет тот же сайт (URL не меняется между итерациями).
- Ограничено 6 публикациями за 5 минут на чат/IP — защищает общий Netlify-аккаунт от исчерпания лимита API одним пользователем.
- Без `NETLIFY_TOKEN` кнопка возвращает `503` с понятным сообщением, остальной функционал не затрагивается.
...
- Значок ▾ рядом с кнопкой публикации открывает историю прошлых публикаций (до 20 записей) с QR-кодом текущего сайта и кнопкой «Снять с публикации» (удаляет сайт на Netlify).
- Статус свежей публикации опрашивается через `GET /api/projects/deploy/:deployId/status`, пока Netlify не пометит её `ready` — кнопка показывает «публикуется…» вместо мгновенного «готово».
