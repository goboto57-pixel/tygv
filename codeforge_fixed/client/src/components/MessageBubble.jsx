...
// errors, etc). Unlike the toast (which auto-dismisses in a few seconds),
// this stays in the chat log and lets you copy the exact request/response
// that failed — status code, headers, bodies — instead of just a one-line
// summary.
function ErrorDetailCard({ message: errMessage, detail, stack }) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const fullText = JSON.stringify({ message: errMessage, detail, stack }, null, 2);
  const copy = () => {
  ...
  return (
    <div className="error-detail-card">
      <div className="error-detail-head">
        <XCircle size={14} color="var(--accent-red, #e5484d)" />
        <span className="error-detail-title">{errMessage}</span>
      </div>
      <div className="error-detail-actions">
        <button className="error-detail-btn" onClick={() => setOpen((o) => !o)}>
          {open ? "Скрыть подробности" : "Показать подробности"}
        </button>
        <button className="error-detail-btn" onClick={copy}>
          <Copy size={12} /> {copied ? "Скопировано" : "Копировать всё"}
        </button>
      </div>
      {open && (
        <pre className="error-detail-body">{fullText}</pre>
      )}
    </div>
  );
}
      ...
      <div className="msg-bubble msg-bubble-assistant">
        ...
        <div className="msg-actions" style={{ display: "flex", gap: 4, marginTop: 8, flexWrap: "wrap" }}>
          <button className="icon-btn" title="Скопировать только код" onClick={async () => {
            ...
            const code = (src.match(/```[\s\S]*?```/g) || []).map((b) => b.replace(/```\w*\n?/, "").replace(/```$/, "")).join("\n\n");
            try { await navigator.clipboard.writeText(code || src); } catch {}
